...
bank_static_power = 0.0000001

leakage_vector = sys.argv[4] # 0.448 * 0.8 * percentBW
...
arr = sys.argv[7].split(',') # 0.448 * 0.8 * percentBW
logic_avg_power = [0 for x in range(16)]
logic_avg_power[0] = float(arr[0])
logic_avg_power[1] = float(arr[1])
logic_avg_power[2] = float(arr[2])
logic_avg_power[3] = float(arr[3])
logic_avg_power[4] = float(arr[4])
logic_avg_power[5] = float(arr[5])
logic_avg_power[6] = float(arr[6])
logic_avg_power[7] = float(arr[7])
logic_avg_power[8] = float(arr[8])
logic_avg_power[9] = float(arr[9])
logic_avg_power[10] = float(arr[10])
logic_avg_power[11] = float(arr[11])
logic_avg_power[12] = float(arr[12])
logic_avg_power[13] = float(arr[13])
logic_avg_power[14] = float(arr[14])
logic_avg_power[15] = float(arr[15])
...
hotspot_tool        =   os.path.expanduser("~/assignment/HotSpot-6.0_SLU_eval_leak_in/hotspot")      #Evaluation without thermal limit
...
#           ptrace_data = ptrace_data + str(round(0,3)) + "\t" 
    if type_of_stack== _HMC:
        for logic in logic_printing_pattern:
            ptrace_data = ptrace_data + str(round(logic_avg_power[logic],3)* 1.0) + "\t"
...
#    bashCommand = "%s -c %s  -f %s -p %s -model_secondary 1 -model_type grid -detailed_3D on -grid_layer_file %s -steady_file %s -grid_steady_file %s" %(hotspot_tool,hotspot_config,hotspot_floorplan,hotspot_ptrace,hotspot_layer_file,hotspot_steady_temp,hotspot_grid_steady)
#    bashCommand = "%s -c %s  -f %s -p %s -o %s -model_secondary 1 -model_type grid  -grid_layer_file %s -steady_file %s -grid_steady_file %s" %(hotspot_tool,hotspot_config,hotspot_floorplan,hotspot_ptrace,hotspot_temp_trace,hotspot_layer_file,hotspot_steady_temp,hotspot_grid_steady)
    bashCommand = "%s -c %s -init_file 4by4.init -f %s -p %s -o %s -model_secondary 1 -model_type grid -detailed_3D on -grid_layer_file %s -steady_file %s -grid_steady_file %s -l %s" %(hotspot_tool,hotspot_config,hotspot_floorplan,hotspot_ptrace,hotspot_temp_trace,hotspot_layer_file,hotspot_steady_temp,hotspot_grid_steady,leakage_vector)
